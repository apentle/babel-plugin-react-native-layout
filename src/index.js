    ...
    visitor: {
      ...
      Program: function Program(path, state) {
        ...
        for (var i = body.length; i--; ) {
          var stm = body[i];
          if (stm.type === 'ExpressionStatement' && stm.expression.type === 'JSXElement') {
            component = false;
            // Transform to layout function
            var actions = stm.expression.children;
              ...
              switch (action.openingElement.name.name) {
                ...
                case 'replace':
                  for (var j = 0, childrenLen = action.children.length; j < childrenLen; j++) {
                    ...
                    if (right !== undefined) {
                      statements.push(t.assignmentExpression('=', element, right));
                      break;
                    }
                  }
                  break;
            ...
            stm.expression = t.assignmentExpression(
              '=',
